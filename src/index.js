...
import {loginUser,cadastrarUser,dadosUser,moneyUser,moneyUserGET} from './controllers.js'
...
// server.post('/cadastro', cadastrarUser)

// server.get('/posts', dadosUser)

// server.post('/moneys', moneyUser)

// server.get('/moneys',moneyUserGET)
...
// }
// async function dadosUser(req,res){
...
// async function moneyUser(req,res){
//     const token=req.headers.authorization?.replace('Bearer ','')
...
// async function moneyUserGET(req,res){
//     const token=req.headers.authorization?.replace('Bearer ','')
//    if (!token){return res.status(401).send('token não encontrado!')}
//     const session = await db.collection('sessions').findOne({token:token})
//     if (!session) { return res.send(401)};
